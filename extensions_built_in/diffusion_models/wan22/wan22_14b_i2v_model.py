...
class Wan2214bI2VModel(Wan2214bModel):
	...
	def generate_single_image(
		...
		if gen_config.ctrl_img is not None:
			...
			# ──────────────────────────────────────────────────────────────
			# Optimized normalization: [0, 255] → [-1, 1] with FP8 support
			control_tensor = TF.to_tensor(control_img).unsqueeze(0).to(
				self.device_torch, dtype=self.torch_dtype
			)

			if self.torch_dtype == torch.float8_e4m3fn and FP8_OPS_AVAILABLE:
				# Fast path: stay in FP8 using custom scalar multiplication
				#  (x / 255) * 2 - 1  =  (x * (2/255)) - 1
				scale = 2.0 / 255.0
				scaled = fp8_ops.scalar_mul_fp8(control_tensor, scale)
				first_frame_n1p1 = fp8_ops.fp8_sub_scalar(scaled, 1.0)
			else:
				# Safe fallback path (works for any dtype)
				first_frame_n1p1 = control_tensor * 2.0 - 1.0
			# ──────────────────────────────────────────────────────────────
