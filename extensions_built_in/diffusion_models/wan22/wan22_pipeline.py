...
class Wan22Pipeline(WanPipeline):
	...
	vae: AutoencoderKLWan,
		...
		# Force VAE to bfloat16 at initialization
		...
		super().__init__(
			...
			vaeb16,  # ← replaced vae with bfloat16 version
			scheduler=scheduler,
		)
	...
	def __call__(
		...
		# unload vae and transformer if aggressive offload
		vaeb_device = self.vae.device
		...
		with self.progress_bar(total=num_inference_steps) as progress_bar:
			for i, t in enumerate(timesteps):
				...
				# apply i2v mask
				latents = (pre_condition_latent_model_input * (1 - mask)) + (latents * mask)
		...
		if self._aggressive_offload:
			...
			if self.transformer_2 is not None:
				self.transformer_2.to("cpu")
			print("Loading Vae")
			self.vae.to(vaeb_device)
			flush()

		if not output_type == "latent":
			# Ensure latents are in bfloat16 before denormalization & decode
			latents = latents.to(dtype=torch.bfloat16)

			latents_mean = (
				torch.tensor(self.vae.config.latents_mean)
				.view(1, self.vae.config.z_dim, 1, 1, 1)
				.to(latents.device, torch.bfloat16)
			)
